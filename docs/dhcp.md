...
!!! abstract "Tujuan & Ruang Lingkup"
    Proyek ini bertujuan untuk membangun dan mengonfigurasi *DHCP Server* (Dynamic Host Configuration Protocol) berbasis Linux (Debian 12) untuk mengotomatisasi pemberian alamat IP dalam sebuah jaringan lokal. Anda akan belajar: Menyiapkan lingkungan server Linux, Menginstal dan mengonfigurasi layanan DHCP untuk Client.
